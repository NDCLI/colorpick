...
def classify_pixel_hsv(rgb):
    ...
    h_deg = h * 360
    if v < 0.18: return "Black (Carbon)", s
    if v > 0.82 and s < 0.22: return "White (Coconut)", s
    if s < 0.25: return "Gray (Silver)", s 
    if h_deg < 15 or h_deg >= 345: res = "Red (Classic)"
    elif 15 <= h_deg < 50: res = "Brown (Dark)" if v < 0.5 else "Orange (Bright)"
    elif 50 <= h_deg < 75: res = "Yellow (Bright)"
    elif 75 <= h_deg < 165: res = "Green (Emerald)"
    elif 165 <= h_deg < 205: res = "Blue (Cyan)"
    elif 205 <= h_deg < 265: res = "Blue (Classic)"
    elif 265 <= h_deg < 320: res = "Purple (Violet)"
