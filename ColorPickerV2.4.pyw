...
# ==========================================
# v2.4 - LOGIC MAU MOI (11 NHOM MAU CHUAN)
...
def classify_pixel(rgb):
    """
    Phan loai 1 pixel vao 1 trong 11 nhom mau.
    ...
    # --- VUNG MAU TRUNG TINH (Achromatic) ---
    # Black: qua toi
    if v < 0.18:
        return "Black", s
    # White: sang + nhat
    ...
    # --- VUNG MAU CO SAC (Chromatic) ---
    # Maroon=#800000: hue=0, v=0.50, s=1.0 -> Brown
    # Brown=#A52A2A: hue=0, v=0.65, s=0.75 -> needs Brown
    # Crimson=#DC143C: hue=348, v=0.86, s=0.91 -> Red
    # HotPink=#FF69B4: hue=330, -> Pink
    # Purple=#800080: hue=300, -> Purple
    # Magenta=#FF00FF: hue=300 -> Pink
    # Pink #FFC0CB: hue=352, v=1.0, s=0.25 -> Pink (low sat)

    # Red zone: 335-360 va 0-20
    if h_deg >= 335 or h_deg < 20:
        # Rose/Pastel Pinks with low hue (e.g. #E69F99 with h~4.7, s~0.33, v~0.9)
        if h_deg < 15 and s <= 0.40 and v >= 0.85:
            return "Pink", s

        # Low sat o red zone = pastel Pink hoac Gray-ish
        if s < 0.30:
            if v > 0.75:
                return "Pink", s  # Pale Pink (FFC0CB)
            return "Red", s
        
        # Keep #8C5568 (h=339, s=0.39) as Pink
        if 335 <= h_deg < 345 and s < 0.60:
            return "Pink", s

        # Low brightness + high saturation = Brown (Maroon, A52A2A)
        # Hue >= 335 is usually Dark Red (Burgundy, #65122D)
        if v < 0.68 and s > 0.65:
            if h_deg >= 335:
                return "Red", s
            return "Brown", s
        return "Red", s

    # Pink / Magenta / HotPink: 290-345
    elif h_deg >= 290:
        # Purple sat cao, v thap -> Purple
        if (h_deg < 315 and v < 0.65 and s > 0.6) or (v < 0.45 and s > 0.4):
            return "Purple", s  # #800080=hue300, v=0.50 hoac mau tim sam (#64304A)
        return "Pink", s

    # Orange + Brown: 20-45
    elif h_deg < 45:
        # Chocolate=#D2691E: hue=25, v=0.82, s=0.81 -> Orange by default
        # SaddleBrown=#8B4513: hue=25, v=0.55, s=0.86 -> Brown (v thap)
        if v < 0.60:
            return "Brown", s
        return "Orange", s

    # Yellow: 45-75 (Gold=#DAA520: hue=43)
    # Gold hue=43 -> Orange zone nen xu ly o Orange
    elif h_deg < 75:
        # Olive/Dark Yellow -> Brown
        if v < 0.55:
            return "Brown", s
        return "Yellow", s
    ...
    # Blue: 165-260 (Cyan, Teal, Turquoise)
    elif h_deg < 260:
        return "Blue", s

    # Purple: 260-290 (Indigo, Violet, BlueViolet)
    else:
        return "Purple", s
...
def engine_saliency(pixels):
    ...
    for p in pixels:
        ...
        # Mau co sac duoc uu ai hon (trong so bao hoa)
        weight = (1.0 + sat * 2.0) if name not in ("Black", "White", "Gray") else 0.8
        scores[name] += weight
...
def engine_kmeans(image, k=4, iterations=8):
    ...
    for i, c in enumerate(centroids):
        ...
        w_val = ratio * (1.0 + sat * 2.0) * (0.7 if name in ("Black", "White", "Gray") else 1.0)
        scores[name] += w_val
...
def analyze_ensemble(image):
    ...
    # 1. Dua tren tan suat pixel (> 10%)
    secondary = []
    for name, cnt in px_counts.most_common():
        if name != final_color and cnt / total_px > 0.10:
            secondary.append(name)
        if len(secondary) >= 2:
            break

    # 2. Dua tren LAB distance (neu co mau sat lai voi mau chinh)
    mean_rgb = tuple(int(round(v)) for v in px.mean(axis=0).astype(float) if True)
    # Trich xuat rieng vi numpy mean tra ve array
...
# ==========================================
# WIN32 HOTKEY LISTENER (Alt+S / Alt+X)
# ==========================================
